...
def state_update(N_up, N, beta, field, dt):
    ...
    m = (2 * N_up - N) / N
    energy_diff_up = -2*(1/N+m+field)
    energy_diff_down = -2*(1/N-m-field)
    ...
    trans_rate_up = dt * (N-N_up) * np.exp(-beta * energy_diff_up / 2)  #rates are linear in N
    trans_rate_down = dt * N_up * np.exp(-beta * energy_diff_down / 2)
    r = random.random()
    # if trans_rate_up + trans_rate_down > 0.5:
...
def compare_N():
    ...
    for n_i in range(N_arr):
        ...
        plotm_t_h(result[:2], h, time_arr, N, omega_0, h_0, beta_0, 10, ratio + 1, variance=True, run_number=run_number)

        axleft.plot(time_arr[l // 2:], result[0][l // 2:], label=('$N = $'+str(N)))
        axright.plot(h_0[l//2:], result[0][l//2:])

    axleft.plot(time_arr[l // 2:], h[l // 2:], label='h', c='black', alpha=0.7)

    fig.suptitle(r'Effect of Spin Number on Magnetization at $\beta=1$, $h_0=0.3$ and $\omega_0=0.02$')
